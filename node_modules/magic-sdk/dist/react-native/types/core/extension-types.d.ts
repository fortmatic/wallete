...
declare type GetAdditionalConfiguration<SDK extends SDKBase, TCustomExtName extends string, TExt extends Extension<string>[] | {
    [P in TCustomExtName]: Extension<string>;
}> = SDK extends SDKBaseReactNative ? MagicSDKReactNativeAdditionalConfiguration<TCustomExtName, TExt> : SDK extends SDKBase ? MagicSDKAdditionalConfiguration<TCustomExtName, TExt> : never;
/**
...
export declare type WithExtensions<SDK extends SDKBase> = {
    new <TCustomExtName extends string, TExt extends Extension<string>[] | {
        [P in TCustomExtName]: Extension<string>;
    }, TExtName extends string = TExt extends Extension<string>[] ? ExtensionNames<TExt> : keyof TExt>(apiKey: string, options?: GetAdditionalConfiguration<SDK, TCustomExtName, TExt>): SDK & {
        [P in TExtName]: TExt extends Extension<string>[] ? Omit<GetExtensionFromName<TExt, P>, 'name' | 'config' | 'init'> : TExt extends {
            [P in TExtName]: Extension<string>;
        } ? Omit<TExt[P], 'name' | 'config' | 'init'> : never;
    };
};
export {};
